    ...
    # --- 模式2: 衛生股長 ---
        ...
        if not df.empty:
            ...
            if cls:
                ...
                # [修改] 不再使用單純的 timedelta(days=3)，改用後面的邏輯判斷
    ...
    # --- 模式3: 後台 ---
        ...
        if pwd == st.secrets["system_config"]["admin_password"]:
            ...
            with tab1: # 成績總表
                ...
                # [新增] 排名模式選擇 (回應需求3)
                rank_mode = st.radio("排名模式", ["全校排名", "分年級排名"], horizontal=True)
                    ...
                    if st.button("🚀 產生全學期總成績結算表"):
                        with st.spinner("正在從雲端下載整學期資料... (這可能需要 30 秒)..."):
                            ...
                            if not full_df.empty:
                                ...
                                # 準備班級資料
                                all_classes_df = pd.DataFrame(structured_classes) # 使用 structured_classes 包含 grade
                                all_classes_df.columns = ["年級", "班級"]
